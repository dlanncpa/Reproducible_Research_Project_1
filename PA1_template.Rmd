...
The 5-minute interval containing the maximum number of steps on average across all of the days in the dataset is `r intervalMean[intervalMean$Steps==maxSteps, 1]`, which averaged `r maxStepsForm`.
...
The total number of missing values in the dataset is `r dataNA`.
...
The imputed data mean is `r imputeMeanSteps` which is a difference of `r meanDiff` from the original data. The median of the imputed data is `r imputeMedianSteps` which is a difference of `r medianDiff`. The total number of steps increased by `r totalDiff` in the imputed data.
